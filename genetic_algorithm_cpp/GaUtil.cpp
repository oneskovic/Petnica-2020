...
vector<double> GaUtil::combine_organisms(Organism* organism1, Organism* organism2, vector<double> mutation_factor_range)
{
    ...
    if (mutation_factor_range[0] != mutation_factor_range[1])
    {
        mutation_vector = rand_util->get_random_array(coefs1.size(),
            mutation_factor_range[0], mutation_factor_range[1], "normal");
    } 
...
vector<vector<double>> GaUtil::get_coeffs_from_best(vector<Organism>* organisms, int total_count, vector<double> mutation_factor_range)
{
